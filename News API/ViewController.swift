...
class ViewController: UIViewController {
    ...
    @IBAction func homeBnClick(_ sender: Any) {
        api_url = "https://newsapi.org/v2/top-headlines?apiKey=" + apiKey + "&pagesize=100&country=ca"
        getArticlesFromApi(urlString: api_url)
        resetText()
    ...
    @IBAction func businessBnClick(_ sender: Any) {
        api_url = "https://newsapi.org/v2/top-headlines?apiKey=" + apiKey + "&pagesize=100&country=ca&category=business"
        getArticlesFromApi(urlString: api_url)
        resetText()
    ...
    @IBAction func technologyBnClick(_ sender: Any) {
        api_url = "https://newsapi.org/v2/top-headlines?apiKey=" + apiKey + "&pagesize=100&country=ca&category=technology"
        getArticlesFromApi(urlString: api_url)
        resetText()
    ...
    @IBAction func healthBnClick(_ sender: Any) {
        api_url = "https://newsapi.org/v2/top-headlines?apiKey=" + apiKey + "&pagesize=100&country=ca&category=health"
        getArticlesFromApi(urlString: api_url)
        resetText()
    ...
    @IBAction func entertainmentBnClick(_ sender: Any) {
        api_url = "https://newsapi.org/v2/top-headlines?apiKey=" + apiKey + "&pagesize=100&country=ca&category=entertainment"
        getArticlesFromApi(urlString: api_url)
        resetText()
...
extension ViewController: UISearchBarDelegate {
    ...
    {
        if let text = searchBar.text?.trimmingCharacters(in: .whitespacesAndNewlines).replacingOccurrences(of: " ", with: "%20") {
...
private extension ViewController {
    ...
    func getArticlesFromApi(urlString: String) {
        ...
        if !networkMonitor.isConnected {
            request = URLRequest(url: url, cachePolicy: .returnCacheDataElseLoad, timeoutInterval: 30) //for access offlline
            DispatchQueue.main.async{ [weak self] in
                self?.showMsg(msg: "No internet connection")
            }
        }else {
            request = URLRequest(url: url, cachePolicy: .reloadIgnoringLocalAndRemoteCacheData, timeoutInterval: 30) //get the latest news without any cache
        }
